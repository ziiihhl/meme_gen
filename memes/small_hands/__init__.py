...
def small_hands(images: list[BuildImage], texts, args):

    user_head = images[0].resize((175, 175)).convert("RGBA") #.circle()
    
    # 初始化帧列表，用于存储每一帧图像
    frames: list[IMG] = []

    positions = [
        (42, 13), (42, 13), (42, 13),  # 1-3
    ]
...
add_meme(
    "small_hands",  # 模板的唯一标识符
    small_hands,  # 处理函数
    min_images=1,  # 需要的最小图片数量
    max_images=1,  # 需要的最大图片数量
    keywords=["小手"],  # 搜索关键词
    date_created=datetime(2025, 8, 12),  # 创建日期
    date_modified=datetime(2025, 8, 12),  # 修改日期
)