...
def picking_flowers(images: list[BuildImage], texts, args):
    ...
    user_head = images[0].resize((212, 212)).convert("RGBA") #.circle()
    
    # 初始化帧列表，用于存储每一帧图像
    frames: list[IMG] = []

    positions = [
        (45, 13), (45, 13), (45, 13), (45, 13), (45, 13),  # 1-4
    ]
...
add_meme(
    "picking_flowers",  # 模板的唯一标识符
    picking_flowers,  # 处理函数
    min_images=1,  # 需要的最小图片数量
    max_images=1,  # 需要的最大图片数量
    keywords=["摘花"],  # 搜索关键词
    date_created=datetime(2025, 8, 16),  # 创建日期
    date_modified=datetime(2025, 8, 16),  # 修改日期
)