...
def kurogames_phrolova_boxer(images: list[BuildImage], texts, args):
    ...
    # 将所有帧保存为GIF，动图平均帧率: 0.1
    return save_gif(frames, 0.1)

add_meme(
    "kurogames_phrolova_boxer",  # 模板的唯一标识符
    kurogames_phrolova_boxer,  # 处理函数
    min_images=1,  # 需要的最小图片数量
    max_images=1,  # 需要的最大图片数量
    keywords=["弗洛洛拳击"],  # 搜索关键词
    tags=MemeTags.wuthering_waves,
    date_created=datetime(2026, 1, 12),  # 创建日期
    date_modified=datetime(2026, 1, 12),  # 修改日期
)