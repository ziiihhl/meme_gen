...
def read_love_letters(images: list[BuildImage], texts, args):

    user_head = images[0].resize((220, 220)).convert("RGBA") #.circle()
    
    # 初始化帧列表，用于存储每一帧图像
    frames: list[IMG] = []

    positions = [
        (64, 7), (64, 7), (64, 7), (64, 7), (64, 7),  # 1-5
    ]
...
add_meme(
    "read_love_letters",  # 模板的唯一标识符
    read_love_letters,  # 处理函数
    min_images=1,  # 需要的最小图片数量
    max_images=1,  # 需要的最大图片数量
    keywords=["看情书"],  # 搜索关键词
    date_created=datetime(2025, 8, 14),  # 创建日期
    date_modified=datetime(2025, 8, 14),  # 修改日期
)